...
# 2. iOS
plutil -replace CFBundleShortVersionString -string "$NEW" "$ROOT/ios/Card/Resources/Info.plist" 2>/dev/null \
  || sedi -E "s|<string>[0-9.]+</string>(\s*<key>CFBundleVersion)|<string>$NEW</string>\1|" \
       "$ROOT/ios/Card/Resources/Info.plist" 2>/dev/null \
  || true

# 3. iOS Share Extension
plutil -replace CFBundleShortVersionString -string "$NEW" "$ROOT/ios/CardShareExtension/Info.plist" 2>/dev/null \
  || true

# 4. watchOS
plutil -replace CFBundleShortVersionString -string "$NEW" "$ROOT/watchos/CardWatch/Resources/Info.plist" 2>/dev/null \
  || true
plutil -replace CFBundleShortVersionString -string "$NEW" "$ROOT/watchos/CardComplication/Info.plist" 2>/dev/null \
  || true
