...
| `APPLE_PROVISIONING_PROFILE_BASE64`   | optional | Wildcard mobileprovision (recommended for DriftNotificationService) |
| `APPLE_TEAM_ID`                       | optional | 10-char team ID     |
| `APPLE_BUNDLE_ID`                     | optional | Override the default bundle id (`com.americangroupllc.drift`) |
...
The default bundle ID is `com.americangroupllc.drift`. Because this app ships with
extensions (`DriftNotificationService`), use a **wildcard** profile such as
`com.americangroupllc.drift.*` so the profile covers the host app and every
