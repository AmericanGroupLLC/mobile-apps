...
The default bundle ID is `com.americangroupllc.card`. Because this app ships with
extensions (`CardShareExtension`), use a **wildcard** profile such as
`com.americangroupllc.card.*` so the profile covers the host app and every
