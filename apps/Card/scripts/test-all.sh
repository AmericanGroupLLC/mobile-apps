...
if [ "$(uname)" = "Darwin" ] && command -v xcodebuild >/dev/null 2>&1; then
  green "▶ CardCore Swift Package tests"
  (
    cd "$ROOT/shared/CardCore"
    xcodebuild test \
      -scheme CardCore-Package \
      -destination 'platform=iOS Simulator,name=iPhone 15,OS=latest' \
      CODE_SIGNING_ALLOWED=NO | xcpretty --simple
  ) || { red "✗ CardCore tests failed"; failures=$((failures+1)); }

  if command -v xcodegen >/dev/null 2>&1; then
    green "▶ iOS simulator build"
    (
      cd "$ROOT/ios" && xcodegen generate
      xcodebuild -project Card.xcodeproj -scheme Card \
    ...
    (
      ...
      xcodebuild -project CardWatch.xcodeproj -scheme CardWatch \
