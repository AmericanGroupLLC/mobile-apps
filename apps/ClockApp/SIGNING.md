...
| `APPLE_BUNDLE_ID`                     | optional | Override the default bundle id (`com.americangroupllc.pocket`) |
...
The default bundle ID is `com.americangroupllc.pocket`. Because this app ships with
extensions (`(none)`), use a **wildcard** profile such as
`com.americangroupllc.pocket.*` so the profile covers the host app and every
