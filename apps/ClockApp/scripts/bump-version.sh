...
# 2. iOS
plutil -replace CFBundleShortVersionString -string "$NEW" "$ROOT/ios/Pocket/Resources/Info.plist" 2>/dev/null \
  || sedi -E "s|<string>[0-9.]+</string>(\s*<key>CFBundleVersion)|<string>$NEW</string>\1|" \
       "$ROOT/ios/Pocket/Resources/Info.plist" 2>/dev/null \
  || true

# 3. watchOS
plutil -replace CFBundleShortVersionString -string "$NEW" "$ROOT/watchos/PocketWatch/Resources/Info.plist" 2>/dev/null \
  || true
plutil -replace CFBundleShortVersionString -string "$NEW" "$ROOT/watchos/PocketComplication/Info.plist" 2>/dev/null \
  || true
