...
if [ -d "$ROOT/android" ]; then
  ...
  cp "$ROOT/android/app/build/outputs/apk/release/app-release.apk"      "$STAGE/Pocket-$TAG.apk"      || true
  cp "$ROOT/android/app/build/outputs/bundle/release/app-release.aab"   "$STAGE/Pocket-$TAG.aab"      || true
  cp "$ROOT/android/wear/build/outputs/apk/release/wear-release.apk"    "$STAGE/Pocket-Wear-$TAG.apk" || true
fi

# Marketing zip
green "▶ Marketing site zip"
zip -j "$STAGE/pocket-web-$TAG.zip" \
...
if [ "$(uname)" = "Darwin" ] && command -v xcodegen >/dev/null 2>&1; then
  ...
  xcodebuild -project "$ROOT/ios/Pocket.xcodeproj" \
    -scheme Pocket -configuration Release -sdk iphonesimulator \
    -destination 'generic/platform=iOS Simulator' \
    -archivePath "$ROOT/ios/build/Pocket.xcarchive" \
    CODE_SIGNING_ALLOWED=NO archive | xcpretty --simple || true
  if [ -d "$ROOT/ios/build/Pocket.xcarchive" ]; then
    (cd "$ROOT/ios/build" && zip -qr "$STAGE/Pocket-$TAG-iOS.xcarchive.zip" Pocket.xcarchive)
  fi

  green "▶ watchOS unsigned archive"
  (cd "$ROOT/watchos" && xcodegen generate)
  xcodebuild -project "$ROOT/watchos/PocketWatch.xcodeproj" \
    -scheme PocketWatch -configuration Release -sdk watchsimulator \
    -destination 'generic/platform=watchOS Simulator' \
    -archivePath "$ROOT/watchos/build/PocketWatch.xcarchive" \
    CODE_SIGNING_ALLOWED=NO archive | xcpretty --simple || true
  if [ -d "$ROOT/watchos/build/PocketWatch.xcarchive" ]; then
    (cd "$ROOT/watchos/build" && zip -qr "$STAGE/Pocket-$TAG-watchOS.xcarchive.zip" PocketWatch.xcarchive)
  fi
fi
