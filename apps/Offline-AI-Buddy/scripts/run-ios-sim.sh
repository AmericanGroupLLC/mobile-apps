#!/usr/bin/env bash
# Boot iPhone 15 simulator, install OfflineAIBuddy.app, launch it, and exit.
# macOS-only.
set -euo pipefail
...
green "▶ Building OfflineAIBuddy.app for simulator"
...
# Side-load model file if it exists locally so onboarding can skip the download.
MODEL_LOCAL="$ROOT/models/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf"
if [ -f "$MODEL_LOCAL" ]; then
  green "▶ Side-loading pre-fetched model into the sim's documents dir"
  APP_DOCS_PARENT=$(xcrun simctl get_app_container "$SIM_UDID" \
    com.americangroupllc.offlineaibuddy data 2>/dev/null || true)
  if [ -n "$APP_DOCS_PARENT" ]; then
    mkdir -p "$APP_DOCS_PARENT/Documents/offlineaibuddy/models"
    cp "$MODEL_LOCAL" "$APP_DOCS_PARENT/Documents/offlineaibuddy/models/" || true
  fi
fi
