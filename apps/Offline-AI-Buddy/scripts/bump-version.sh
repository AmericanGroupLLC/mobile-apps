...
# 2. iOS
plutil -replace CFBundleShortVersionString -string "$NEW" "$ROOT/ios/OfflineAIBuddy/Resources/Info.plist" 2>/dev/null \
  || sedi -E "s|<string>[0-9.]+</string>(\s*<key>CFBundleVersion)|<string>$NEW</string>\1|" \
       "$ROOT/ios/OfflineAIBuddy/Resources/Info.plist" 2>/dev/null \
  || true
