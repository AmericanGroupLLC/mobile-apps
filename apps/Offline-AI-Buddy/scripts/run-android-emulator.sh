...
# Side-load model if pre-fetched locally to skip the first-launch flow.
MODEL_LOCAL="$ROOT/models/Qwen2.5-1.5B-Instruct-Q4_K_M.gguf"
if [ -f "$MODEL_LOCAL" ]; then
  green "▶ Side-loading model"
  "$ADB" push "$MODEL_LOCAL" \
    /sdcard/Android/data/com.americangroupllc.offlineaibuddy/files/offlineaibuddy/models/ || true
fi

green "▶ Launching com.americangroupllc.offlineaibuddy"
"$ADB" shell monkey -p com.americangroupllc.offlineaibuddy -c android.intent.category.LAUNCHER 1
