...
| `APPLE_PROVISIONING_PROFILE_BASE64`   | optional | Wildcard mobileprovision (recommended for OfflineAIBuddyKeyboard) |
| `APPLE_TEAM_ID`                       | optional | 10-char team ID     |
| `APPLE_BUNDLE_ID`                     | optional | Override the default bundle id (`com.americangroupllc.offlineaibuddy`) |
...
The default bundle ID is `com.americangroupllc.offlineaibuddy`. Because this app ships with
extensions (`OfflineAIBuddyKeyboard`), use a **wildcard** profile such as
`com.americangroupllc.offlineaibuddy.*` so the profile covers the host app and every
