...
Offline AI Buddy is offline-first. There is no Sentry DSN, no init call,
no SDK binary in the v1 .ipa or .aab. This document is a forward-looking
spec for v1.1 only.

## Why we kept the stub

The `canImport`-gated `CrashReportingService` stub from the Card / Drift
/ BuddyPlay scaffold is preserved so v1.1 can wire Sentry up without an
...
3. Scrub PII: `beforeSend` strips chat content, profile names, PIN
   hashes from the event payload.
4. Update `PRIVACY.md` to disclose what's sent and link to Sentry's
   privacy doc.
5. Set DSN via env var (CI secret), not hard-coded — same pattern Drift
   uses.

## What stays no-op forever

- **Replays**: never. Our screen content includes user chat content.
- **User identification**: never. Profile IDs are random UUIDs scoped
  to the device.
- **Performance traces**: only via the Settings opt-in; default OFF.
...
- `release.yml` does NOT set `SENTRY_DSN_*` env vars in v1. (Slot
  exists for v1.1.)
- `ios/project.yml` does NOT add the Sentry SPM package.
- `android/app/build.gradle.kts` does NOT add the
  `io.sentry:sentry-android` dependency.
