...
Offline AI Buddy's release pipeline is `release.yml`, gated on
`pre-release-tests.yml`. Cutting a release is one tag push.

## 0. Pre-flight

- All keystone tests green locally: `./scripts/test-all.sh`
- Manual smoke checklist passed on a real phone (see `TESTING.md`).
- Marketing site preview reads correctly: `python3 -m http.server`.
- `MODELS.md` SHA-256 entry matches the GGUF you downloaded.
...
Pushing the tag triggers `release.yml`. Pre-release tests must pass
before any binary is built or uploaded.
...
| `build-ios` | Build unsigned `OfflineAIBuddy.app` for iPhone Sim. | pre-release-tests |
...
| `PLAY_STORE_PACKAGE_NAME` | `com.americangroupllc.offlineaibuddy` |
...
| `REVENUECAT_API_KEY_IOS`, `REVENUECAT_API_KEY_ANDROID` | RevenueCat init. Workflow skips RevenueCat-related steps when unset. |
| `ADMOB_APP_ID_IOS`, `ADMOB_APP_ID_ANDROID` | AdMob init. Workflow skips AdMob-related steps when unset. |
...
These are flagged as pre-releases on GitHub and routed to the `beta`
...
git commit -am 'fix(chat): Hindi token streaming regression'
git tag v1.2.1
...
## 8. Re-pinning the llama.cpp submodule

Bumping `vendor/llama.cpp` is its own commit, reviewed separately from a
release:

```sh
cd vendor/llama.cpp
git fetch && git checkout <new-commit-sha>
cd ../..
# Update MODELS.md §1 with the new pin + verify CI builds.
git commit -am 'chore(deps): bump llama.cpp to <new-commit-sha>'
```

Re-run `./scripts/test-all.sh` to verify both platforms still build.
